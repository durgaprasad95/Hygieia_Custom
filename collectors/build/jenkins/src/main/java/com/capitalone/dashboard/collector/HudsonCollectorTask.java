...
public class HudsonCollectorTask extends CollectorTask<HudsonCollector> {
	...
	public HudsonCollectorTask(TaskScheduler taskScheduler, HudsonCollectorRepository hudsonCollectorRepository,
			HudsonJobRepository hudsonJobRepository, BuildRepository buildRepository,
			CollItemCfgHistRepository configRepository, HudsonClient hudsonClient, HudsonSettings hudsonSettings,
	...
	public void collect(HudsonCollector collector) {
		...
		for (String instanceUrl : collector.getBuildServers()) {
			...
			try {
				...
				activeServers.remove(instanceUrl); // since it was a rest exception, we will not delete this job and
													// wait for
	...
	private void clean(HudsonCollector collector, List<HudsonJob> existingJobs) {
		...
		for (HudsonJob job : existingJobs) {
			if ((job.isEnabled() && !uniqueIDs.contains(job.getId())) || // if it was enabled but not on a dashboard
					(!job.isEnabled() && uniqueIDs.contains(job.getId()))) { // OR it was disabled and now on a
																				// dashboard
	 ...
	 * @param collector
	 */
	private void deleteUnwantedJobs(List<HudsonJob> activeJobs, List<HudsonJob> existingJobs,
			List<String> activeServers, HudsonCollector collector) {
		...
		for (HudsonJob job : existingJobs) {
			...
			// if we have a collector item for the job in repository but it's build server
			// is not what we collect, remove it.
			if (!collector.getBuildServers().contains(job.getInstanceUrl())) {
				deleteJobList.add(job);
			}

			// if the collector id of the collector item for the job in the repo does not
			// match with the collector ID, delete it.
			if (!job.getCollectorId().equals(collector.getId())) {
				deleteJobList.add(job);
			}

			// this is to handle jobs that have been deleted from build servers. Will get
			// 404 if we don't delete them.
	 ...
	 * @param dataByJob
	 *            maps a {@link HudsonJob} to a map of data with {@link Build}s.
	 */
	private void addNewBuilds(List<HudsonJob> enabledJobs,
		...
		for (HudsonJob job : enabledJobs) {
			if (job.isPushed())
				continue;
			// process new builds in the order of their build numbers - this has implication
			// to handling of commits in BuildEventListener
			...
			builds.sort((BaseModel b1, BaseModel b2) -> Integer.valueOf(((Build) b1).getNumber())
					- Integer.valueOf(((Build) b2).getNumber()));
			for (BaseModel buildSummary : builds) {
				if (isNewBuild(job, (Build) buildSummary)) {
					Build build = hudsonClient.getBuildDetails(((Build) buildSummary).getBuildUrl(),
							job.getInstanceUrl());
					if (build != null) {
	...
	private void addNewConfigs(List<HudsonJob> enabledJobs,
		...
		for (HudsonJob job : enabledJobs) {
			...
			configs.sort((BaseModel b1, BaseModel b2) -> new Date(((CollItemCfgHist) b1).getTimestamp())
					.compareTo(new Date(((CollItemCfgHist) b2).getTimestamp())));
	...
	private boolean isNewConfig(HudsonJob job, CollItemCfgHist config) {
		return configRepository.findByCollectorItemIdAndJobAndTimestamp(job.getId(), config.getJob(),
