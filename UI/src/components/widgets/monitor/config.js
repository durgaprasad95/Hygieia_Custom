(function() {
	...
	angular.module(HygieiaConfig.module).controller('monitorConfigController',
			monitorConfigController);

	monitorConfigController.$inject = [ '$scope', '$q', '$uibModalInstance',
			'monitorData', 'modalData' ];
	function monitorConfigController($scope, $q, $uibModalInstance,
			monitorData, modalData) {
				...
				function getDependentServices(services) {
					if (services) {
						for (var x = 0; x < services.length; x++) {
							...
							services[x].name = item.applicationName + ': '
									+ item.name;
						}
					}
		...
		function processSearchResponse(response) {
			...
			ctrl.allServices = _(response).filter(function(item) {
		...
		function deleteDashboardService(idx) {
			deletedDashboardServices
					.push(ctrl.dashboardServices.splice(idx, 1)[0]);
		}

		function addNewDashboardService() {
			ctrl.newDashboardServices.push({
				name : ''
			});
		}
		...
		function deleteDependentService(idx) {
			deletedDependentServices
					.push(ctrl.dependentServices.splice(idx, 1)[0]);
		}

		function addNewDependentService() {
			ctrl.newDependentServices.push({
				selectedItem : {}
			});
		}
		...
		function save() {
			...
			_(deletedDependentServices).forEach(
					function(item) {
						promises.push(monitorData.deleteDependentService(
								dashboardId, item.id));
					});

			whereName(ctrl.newDashboardServices).uniq(function(item) {
				return item.name.toLowerCase();
			}).forEach(
					function(item) {
						promises.push(monitorData.createService(dashboardId,
								item.name, item.url));
					});
			...
			whereName(_.map(ctrl.newDependentServices, function(item) {
				return item.selectedItem;
					...
					function(item) {
						promises.push(monitorData.createDependentService(
								dashboardId, item.id));
					});
			...
			$q
					...
					.then(
							function(responses) {
								...
								$uibModalInstance
										.close(responses.length ? widgetResponse
												: null);
							}, function(response) {
