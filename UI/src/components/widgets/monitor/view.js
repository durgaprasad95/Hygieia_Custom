(function() {
	...
	angular.module(HygieiaConfig.module).controller('monitorViewController',
			monitorViewController).controller('monitorStatusController',
			monitorStatusController);

	monitorViewController.$inject = [ '$scope', 'monitorData', 'DashStatus',
			'$uibModal', '$q', '$interval' ];
	function monitorViewController($scope, monitorData, DashStatus, $uibModal,
			$q, $interval) {
		...
		$interval(function() {
			...
			for (var i = 0; i < ctrl.services.length; i++) {
				monitorData.refreshService($scope.dashboard.id,
						ctrl.services[i]);
			}
		...
		function openStatusWindow(service) {
			...
			$uibModal.open({
			...
			}).result.then(function(updatedService) {
				// if the window is closed without saving updatedService will be
				// null
				if (!updatedService) {
					return;
				}
				...
				// update locally
				_(ctrl.services).forEach(
						function(service, idx) {
							if (service.id == updatedService.id) {
								ctrl.services[idx] = angular.extend(service,
										updatedService);
							}
						});
				...
				return _
						.map(
								...
								function(item) {
									...
									if (item.status
											&& (typeof item.status == 'string' || item.status instanceof String)) {
		...
		function workerCallback(data) {
			...
			var length = ctrl.services.length;
			ctrl.rightServices = [];
			ctrl.leftServices = [];
			for (var i = 0; i < length; i++) {
				if (length % 2 == 0) {
					if (i < length / 2) {
						ctrl.leftServices.push(ctrl.services[i]);
					} else {
						ctrl.rightServices.push(ctrl.services[i]);
					}
				} else {
					if (i <= length / 2) {
						ctrl.leftServices.push(ctrl.services[i]);
	...
	monitorStatusController.$inject = [ 'service', 'statuses',
			'$uibModalInstance' ];
