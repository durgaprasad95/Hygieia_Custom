(function() {
	...
	function featureConfigController(modalData, $uibModalInstance,
			collectorData, featureData) {
		...
		ctrl.teams = [];
		ctrl.teamId = widgetConfig.options.projectId;
		ctrl.teamName = widgetConfig.options.projectName;
		ctrl.featureTypeOption = "";
		ctrl.featureTypeOptions = [];
		ctrl.estimateMetricType = "";
		ctrl.estimateMetrics = [ {
			type : "hours",
			value : "Hours"
		}, {
			type : "storypoints",
		...
		function processCollectorsResponse(data) {
			...
			function getCollectors(data, currentCollectorId) {
				...
				if ((ctrl.selectedTypeIndex === undefined)
						|| (ctrl.selectedTypeIndex === null)) {
					ctrl.collectorId = '';
					ctrl.hideProjectDropDown = true;
					ctrl.hideTeamDropDown = true;
					ctrl.hideSprintTypeDropDown = true;
					...
					if (ctrl.collectorId.value === 'Jira' || 'Rally') {
		...
		function initProjectName(widgetConfig) {
			...
			if (widgetConfig.options.projectName != undefined
					&& widgetConfig.options.projectName != null) {
				ctrl.projectName = widgetConfig.options.projectName;
			}
		}

		function initTeamName(widgetConfig) {
			if (widgetConfig.options.teamName != undefined
					&& widgetConfig.options.teamName != null) {
				if (ctrl.collectorId.value != 'Rally') {
		...
		function initEstimateMetricType(widgetConfig) {
			if (widgetConfig.options.estimateMetricType != undefined
					&& widgetConfig.options.estimateMetricType != null) {
		...
		function initSprintType(widgetConfig) {
			if (widgetConfig && widgetConfig.options
					&& widgetConfig.options.sprintType) {
		...
		function initSelectedProjectAndTeam(widgetConfig) {
			if (widgetConfig && widgetConfig.options) {
				ctrl.selectedProjectObject = {
					...
					pId : widgetConfig.options.projectName === 'Any' ? 'Any'
							: widgetConfig.options.projectId
				}
				ctrl.selectedTeamObject = {
					name : widgetConfig.options.teamName,
					teamId : widgetConfig.options.teamName === 'Any' ? 'Any'
							: widgetConfig.options.teamId
				}
			}
		}
		...
		function getProjectNames(filter) {
			return featureData.projectsByCollectorIdPaginated(
					ctrl.collectorId.id, {
						"search" : filter,
						"size" : 20,
						"sort" : "description",
				...
				if (!angular.isUndefined(filter) && filter.match(/^t/gi)) {
					var defaultValue = {
						name : 'Any',
						value : 'Any',
						pId : 'Any',
		...
		function getTeamNames(filter) {
			return featureData.teamsByCollectorIdPaginated(ctrl.collectorId.id,
					{
						"search" : filter,
						"size" : 20,
						"sort" : "description",
		...
		function createCollectorItem() {
			...
			} else if (ctrl.collectorId.value === 'Rally') {
				collectorId = _.find(ctrl.collectors, {
					name : 'Rally'
				}).id
				item = createItemFromSelect(collectorId)
		...
		function processCollectorItemResponse(response) {
			var postObj = {
				...
				options : {
					...
					showStatus : { // starting configuration for what is
						// currently showing. Needs to be mutually
						// exclusive!
						kanban : "kanban" === ctrl.sprintType
								|| "scrumkanban" === ctrl.sprintType,
