...
(function() {
	...
	angular.module(HygieiaConfig.module).controller(
			'BuildWidgetViewController', BuildWidgetViewController);

	BuildWidgetViewController.$inject = [ '$scope', 'buildData',
			'DisplayState', '$q', '$uibModal' ];
	function BuildWidgetViewController($scope, buildData, DisplayState, $q,
			$uibModal) {
		var ctrl = this;
		var builds = [];
		...
		ctrl.lineOptions = {
			plugins : [ Chartist.plugins.gridBoundaries(),
					Chartist.plugins.lineAboveArea(),
					Chartist.plugins.tooltip(), Chartist.plugins.pointHalo() ],
			showArea : true,
			lineSmooth : false,
			fullWidth : true,
			chartPadding : 7,
		...
		function processResponse(data) {
			...
			function averageBuildDuration(data, buildThreshold, cb) {
				...
				function filter(data) {
					return _.filter(data, function(item) {
						return item.buildStatus == 'Success'
								&& Math.floor(moment(item.endTime).endOf('day')
				...
				function simplify(data) {
					// create array with date as the key and build duration
					// times in an array
					var simplifiedData = {};
					_.forEach(data, function(buildDay, key) {
						...
						_.forEach(buildDay, function(build) {
							var duration = moment(build.endTime).diff(
									moment(build.startTime), 'seconds') / 60;
				...
				function getPassFail(simplifiedData) {
					...
					var passed = [], failed = [];
					for (var x = 0; x <= 14; x++) {
						var date = moment(new Date()).subtract(x, 'days')
								.format('L');
			...
			function buildsPerDay(data, cb) {
				...
				function countBuilds(data) {
					...
					_.times(15, function() {
						var count = grouped[dt.getTime()] ? grouped[dt
								.getTime()].length : 0;
						counts.push(count);
			...
			function latestBuilds(data, cb) {
				...
				data = _.map(data, function(item) {
					return {
						...
						url : item.buildUrl,
						release : item.codeRepos[0].branch.split("_")[1]
					};
				});
			...
			function setDisplayToErrorState(data, failureThreshold, cb) {
				// order by end time and limit to last 5
				data = _.sortBy(data, 'endTime').reverse().slice(0,
						failureThreshold);
				data = _
						.filter(
								data,
								function(item) {
									return (item.buildStatus.toLowerCase() != 'success')
											&& (item.buildStatus.toLowerCase() != 'inprogress');
								});
			...
			// call to webworker methods nad set the controller variables with
			...
			worker.averageBuildDuration(data,
					$scope.widgetConfig.options.buildDurationThreshold,
			...
			worker
					.setDisplayToErrorState(
							data,
							$scope.widgetConfig.options.consecutiveFailureThreshold,
							function(displayAsErrorState) {
								// $scope.$apply(function () {
								$scope.display = displayAsErrorState ? DisplayState.ERROR
